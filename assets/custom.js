...
let shopCommon = {
  ...
  countTime: function(el, targetDate) {
    let me = this;

    //time different, d,h,m,s save countdown
    var leftTime = targetDate - me.nowDate, d, h, m, s;
    if (leftTime >= 0) {
      ...
      el.innerHTML = String(d).padStart(2, "0") + '<i>D</i>' + String(h).padStart(2, "0") + '<i>H</i>' + String(m).padStart(2, "0") + '<i>M</i>' + String(s).padStart(2, "0") + '<i>S</i>';
      setTimeout(function() { me.nowDate += 1000; me.countTime(el, targetDate) }, 1000)
    } else {
      el.style = "display: none"
    }
  },

  stratActiveCountDown: function() {
    let me = this;
    $.getJSON("https://quan.suning.com/getSysTime.do", function(r) {
      var _t = r.sysTime2.replace(/-/g,'/');
      // me.nowDate = CURRENT_TIME = new Date(_t).getTime() - 54000000; // US western time
      me.nowDate = new Date(_t).getTime(); // CHINA time

      document.querySelectorAll('.countdown').forEach(item => {
        // 2021/10/05 16:00:00
        let t = item.dataset.time;
        me.verifyDateFormat(t) && me.countTime(item, new Date(t).getTime())
      })
    })
  },

  getQueryParams: function(searchParams) {
    const params = new URLSearchParams(window.location.search);
    return params.get(searchParams)
  }
}

shopCommon.stratActiveCountDown()