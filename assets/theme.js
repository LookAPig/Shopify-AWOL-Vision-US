...
`;
  ...
  var ProductVariants = class extends CustomHTMLElement {
    async connectedCallback() {
      console.log('this.getAttribute("form-id")', document.getElementById(this.getAttribute("form-id")).id)
    ...
    selectVariant(id) {
      ...
      console.log("selectVariant", this.selectedVariant);
      window.localStorage.setItem('v_id', this.selectedVariant.id)
      $(".shipping_item").hide()
      ...
      // change affirm
      if (affirm) {
        $('.affirm-as-low-as').attr('data-amount', this.selectedVariant.price);
        affirm.ui.refresh()
      }
    }
    _onOptionChanged() {
  ...
  var ProductStickyForm = class extends HTMLElement {
    ...
    _setupVisibilityObservers() {
      var element = document.getElementById("product-tabs"); // 替换为您的元素ID
      ...
      this.intersectionObserver = new IntersectionObserver((entries) => {
        entries.forEach((entry) => {
          ...
          if (entry.target === element) {
            ...
            const boundingRect2 = element.getBoundingClientRect()
            console.log('boundingRect2', boundingRect2)
            console.log('stickyHeaderOffset', stickyHeaderOffset)
            this._isPaymentContainerPassed = entry.intersectionRatio === 0 && boundingRect2.top + boundingRect2.height <= stickyHeaderOffset;
          }
        });
        if (window.matchMedia(window.themeVariables.breakpoints.pocket).matches) {
