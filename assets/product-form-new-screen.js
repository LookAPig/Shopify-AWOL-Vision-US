(function () {
  console.log("screen 模板 产品是应该是 >>>>>>>>>>>>", location.href);
  ...
  function filterAndUpdateVariantIds(data) {
    return data.map((item_1) => {
      ...
      for (const item_2 of data) {
        if (item_1.option1 === item_2.option1) {
          variantIds.push(item_2.id);
        }
      }

      // 更新原对象的 variant_ids 字段
      item_1.featured_image.variant_ids = variantIds;
  ...
  const handelSelected = () => {
    ...
    variantLabelEles.forEach((variantEle) => {
      ...
      updateVariants.forEach((updateVariantItem, updateVariantIndex) => {
        // console.log(
        //   "updateVariantItem >>>>>>>>>>>>>>",
        //   updateVariantItem.name,
        //   "选项文本>>>>>>>>>>",
        //   firstLevelText,
        //   "bool",
        //   updateVariantItem.name === firstLevelText
        // );

        const updateStr = updateVariantItem.name.replace(/ {2,}/g, " ");

        const selectStr = firstLevelText.replace(/ {2,}/g, " ");

        if (updateStr !== selectStr) return;
  ...
  document.querySelector(
    ...
    console.log("Add cart", this)
    let variantId = document.querySelector('[oid="add-to-cart"]').id.value;

    const quantity = document.querySelector(".quantity-selector__input").value;

    const variantParams = [
      {
        variant_id: variantId ? Number(variantId) : 1,
    ...
    fetch(window.Shopify.routes.root + "wallets/checkouts.json", {
      ...
      headers: {
        ...
        Authorization: "Basic ZDgwNDg0MWQzZTkzNmNhMmY2NGIyODlmZGFmMTZhYjg=",
      },
      body: JSON.stringify({
        checkout: {
          line_items: variantParams,
          secret: true,
